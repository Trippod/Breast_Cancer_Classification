...
PKLDIR = ROOT / "models"
RESDIR = ROOT / "outputs" / "models_res" / "knn"
...
def train_knn() -> None:
    ...
    clf.fit(X_tr, y_tr)
    joblib.dump(clf, PKLDIR / "knn.pkl")

    y_pred  = clf.predict(X_te)
    # predict_proba dostępne w KNN
    y_proba = clf.predict_proba(X_te)[:, 1]
    auc     = roc_auc_score(y_te, y_proba)

    _report_and_plot("K-Nearest Neighbors", y_te, y_pred, auc,
                     RESDIR / "confusion_matrix.png")
    print(f"✓ model  → {PKLDIR/'knn.pkl'}")
